...
if (!class_exists('DBConnection')) {
    require_once('../config.php'); // Ensure you include your config
    require_once('DBConnection.php');
}

class SystemSettings extends DBConnection {
    public function __construct() {
        parent::__construct(); // Call the parent constructor to initialize DB connection
    }

    function check_connection() {
        return $this->conn; // Check the connection
    }

    function load_system_info() {
        if (!isset($_SESSION['system_info'])) {
            $sql = "SELECT * FROM system_info";
            $qry = $this->query($sql); // Use the query method from DBConnection
    ...
    function update_settings_info() {
        foreach ($_POST as $key => $value) {
            if (!in_array($key, array('about', 'welcome_message'))) {
                if (isset($_SESSION['system_info'][$key])) {
                    $this->query("UPDATE system_info SET meta_value = '{$this->escape($value)}' WHERE meta_field = '{$this->escape($key)}'");
                } else {
                    $this->query("INSERT INTO system_info SET meta_value = '{$this->escape($value)}', meta_field = '{$this->escape($key)}'");
                }
            }
        }
        ...
        if (isset($_FILES['img']) && $_FILES['img']['tmp_name'] != '') {
            ...
            if (isset($_SESSION['system_info']['logo'])) {
                $this->query("UPDATE system_info SET meta_value = '{$this->escape($fname)}' WHERE meta_field = 'logo'");
                if (is_file('../' . $_SESSION['system_info']['logo'])) {
                    unlink('../' . $_SESSION['system_info']['logo']);
                }
            } else {
                $this->query("INSERT INTO system_info SET meta_value = '{$this->escape($fname)}', meta_field = 'logo'");
            }
        }

        if (isset($_POST['welcome_message'])) {
            if (!is_file(base_app . 'welcome.html')) {
                file_put_contents(base_app . 'welcome.html', ''); // Create an empty file if it doesn't exist
            }
            file_put_contents(base_app . 'welcome.html', $_POST['welcome_message']);
        }
    ...
    function flashdata($flash = '') {
        if (!empty($flash)) {
            $_tmp = $_SESSION['flashdata'][$flash];
            unset($_SESSION['flashdata'][$flash]); // Unset only the requested flash data
            return $_tmp;
        } else {
            return false;
        }

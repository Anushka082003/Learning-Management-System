...
/*<?php if ($_settings->chk_flashdata('success')): ?>
    ...
    <?php
    $id = isset($_GET['id']) ? intval($_GET['id']) : ''; // Validate and sanitize the ID
    if (!empty($id)) {
        ...
        // Fetch the result
        if ($qry->num_rows > 0) { // Check if any rows were returned
            foreach ($qry->fetch_array() as $k => $v) {
                if (!is_numeric($k)) {
                    $$k = $v; // Use variable variables to assign values dynamically
                }
            }
            $description = stripslashes($description);
        } else {
            // Handle case where no lessons are found
            die("No lesson found with the given ID.");
        }
    }
    ?>
            ...
            <div id="carousel_holder" class="w-100">
                <div id="lesson_slides" class="carousel slide" data-ride="carousel" data-interval="0">
                    <div class="carousel-inner">
                        ...
                        foreach ($slides as $k => $v) {
                            if (in_array($slides[$k], array('.', '..'))) {
                                unset($slides[$k]); // Remove current and parent directory from the array
                            }
                        }
                        $active = "active";
                        foreach ($slides as $k => $v):
